...
Then read `../growth-engine/SKILL.md`. It's a mechanics layer on top of voice.md — hook-generation process, tension/payoff sequencing, proof-over-claims, visual strategy — that applies to every archetype, not just this one. voice.md still wins on anything the two disagree on.
...
3. **Pivot line**: explicitly name that most people are missing the real story. This is where opinion starts. Internally, work through: what happened → what everyone else covering this will report → what actually matters → who this changes things for. The pivot line is the compressed output of that chain, not all of it spelled out.
